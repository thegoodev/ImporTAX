...
Widget buildButton(String _tag, void _func(), double t, double b){
  ...
  return Padding(
    ...
    child:RaisedButton(
      child: Padding(padding: EdgeInsets.symmetric(vertical: 16.0), child: Text(_tag, style: TextStyle(fontSize: 16.0))),
...
class _HomePageState extends State<HomePage> {
  ...
  void _agregarProducto(bool standAlone){
    ...
    Navigator.push(context, MaterialPageRoute( 
      builder: (context)=> ProductSelector(standAlone: standAlone, data: loadData(),),
  ...
  Widget _buildtitle(){
    ...
    if(_total != 0){
      return Text("\$ "+f.format(_total), style: numStyle.copyWith(fontSize: 18));
    }else{
  ...
  Widget _buildImpuesto(){
    ...
    if(widget.items.length >= 2){
      ...
      if(_impTotal.total != 0){
        return Padding(
        padding: EdgeInsets.only(top: 16.0, bottom: 8, left: 16.0, right: 16.0),
        child: Row(
          mainAxisSize: MainAxisSize.max,
          mainAxisAlignment: MainAxisAlignment.spaceAround,
          children: <Widget>[
              Expanded(child: Text("Impuesto", style: bigger.copyWith(color: Colors.blue),),),
              Text(f.format(_impTotal.total), style: sub.copyWith(fontSize: 18.0, color: Colors.blue),)
            ],
          ),
        );
      }
      ...
      return Padding(
        ...
        child: Text("Oh, no!", style: TextStyle(fontSize: 24.0),),
      );
    }
  }
  ...
  Widget _buildCIF(){
    ...
    if (widget.items.isEmpty){
      return Padding(
        ...
        child: Text("Aún no se ha escogido ningún producto, para empezar a calcular elige un producto")
      );
      ...
      if(_cifTotal.total != 0){
        return Padding(
        padding: EdgeInsets.only(top: 8.0, bottom: 0, left: 16.0, right: 16.0),
        child: Row(
          mainAxisSize: MainAxisSize.max,
          mainAxisAlignment: MainAxisAlignment.spaceAround,
          children: <Widget>[
              Expanded(
                child: Column(
                  ...
                  children: <Widget>[
                    Text("CIF", style: bigger.copyWith(color: Colors.green),),
                    Text("Sin impuesto", style:sub)
                  ],
                ),
              ),
             Text(f.format(_cifTotal.total), style:sub.copyWith(fontSize: 18.0, color: Colors.green))
            ],
          ),
        );
      }
  ...
  Widget build(BuildContext context) {
    ...
    if(removalP != null && removalS != null){
      Summary s = widget.items.firstWhere((s)=> s.id==removalS, orElse: ()=> null);
      if(s != null){
    ...
    return Scaffold(
        body: CustomScrollView(
          slivers: <Widget>[
            ...
            SliverToBoxAdapter(
              child: _buildImpuesto()
            ),
            SliverToBoxAdapter(
              child: _buildCIF(),
            ),
            SliverPadding(
              padding: (widget.items.length==1 && widget.items[0].productos.length ==1)?EdgeInsets.only(bottom: 12.0):EdgeInsets.all(12.0),
              sliver: SliverList(
              delegate: SliverChildBuilderDelegate(
                (BuildContext context, int i){
                  if(widget.items.length == 1){
                    if(widget.items[0].productos.length == 1){
                      return buildDetail(widget.items[0].productos[0], true, context, widget.prefs, widget.items[0].id);
                    }else{
            ...
            SliverToBoxAdapter(
                child:Padding(
                  ...
                  child: buildButton("Agregar Producto", (){_agregarProducto(false);},8,0)),
              ),
            SliverToBoxAdapter(
              child: Opacity(
                ...
                child:
                FlatButton(
                      child: Text('o empezar de cero', style: TextStyle(color: Colors.red[300]),),
                      onPressed: (){ _agregarProducto(false); widget.items.clear(); _calculateTotal(); widget.prefs.setStringList('items', []);}, 
                    )
              ))
          ],
        )
