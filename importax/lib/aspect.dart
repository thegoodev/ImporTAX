...
class ProductState extends State<Product> {
  ...
  Widget _cif(bool single) {
    return Text(
      (single)
          ? f.format(widget.producto.cif.total)
          : widget.producto.cif.getValNi(),
    ...
    return BlockContainer(
      ...
      onTap: () {
        Navigator.push(
            ...
            MaterialPageRoute(
                builder: (context) =>
                    Detail(widget.producto, widget.prefs, widget.sumId)));
      },
    );
  }
}
      ...
      child: Row(
        ...
        children: <Widget>[
          ...
          Padding(
            ...
            child: Text(f.format(widget.sumary.impuesto.total),
                style: numStyle.copyWith(fontSize: 16, color: Colors.blue)),
          ),
          Text(f.format(widget.sumary.cif.total),
    ...
    if (widget.summary.productos.length == 1) {
      body = Column(
        ...
        children: <Widget>[
          ...
          Product(
            producto: widget.summary.productos[0],
            single: true,
            prefs: widget.prefs,
            sumId: widget.summary.id,
          )
        ],
      );
    } else {
      body = Column(
        ...
        children: <Widget>[
          Header(sumary: widget.summary),
          ListBody(
            children: widget.summary.productos
                .map((p) => Product(
                      producto: p,
                      single: false,
                      prefs: widget.prefs,
            ...
            children: <Widget>[
              Padding(
                  padding: EdgeInsets.only(
                      left: 8.0, right: (icon == null) ? 0.0 : 8.0),
                  child: icon),
              Expanded(
                child: Container(
                  padding: EdgeInsets.only(
                      left: 8.0, right: 16, bottom: 12.0, top: 8.0),
                  decoration: border
                      ? BoxDecoration(
                          border: BorderDirectional(
    ...
    return Column(
        ...
        children: <Widget>[
          ...
          BlockContainer(
            border: true,
            body: Text(
              "Impuesto Selectivo al Consumo (" + p.info.isc.toString() + "%)",
              style: sub,
            ),
...
Widget buildDetail(Producto producto, bool single, BuildContext context,
    SharedPreferences prefs, String sumID) {
    ...
    children: <Widget>[
      BlockContainer(
        ...
        body: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: <Widget>[
            Text(producto.info.nombre,
                style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
            Text(producto.info.partida,
        ...
        trailing: (single)
            ? IconButton(
                ...
                onPressed: () {
                  deleteProduct(producto, context, prefs, sumID, single);
                },
              )
            : null,
      ),
      Padding(
          ...
          child: Row(
            ...
            children: <Widget>[
              Text('Impuesto ' + '(' + pctTotal.toStringAsFixed(2) + '%)',
      ...
      Padding(
          ...
          child: Row(
            ...
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: <Widget>[
              Text('CIF' + '(' + (100 - pctTotal).toStringAsFixed(2) + '%)',
      ...
      BlockContainer(
        border: true,
        body: Text(
          "Seguro de viaje (" + producto.cif.pct.toStringAsFixed(2) + "%)",
          style: sub,
        ),
        ...
        child: Padding(
            ...
            child: Row(
              ...
              children: <Widget>[
                ...
                Expanded(
                  child: Text(
                    "Este producto podría estar restringido, has click aquí para averiguarlo",
...
void deleteProduct(Producto producto, BuildContext context,
    SharedPreferences prefs, String sumID, bool single) {
  showDialog(
    context: context,
    builder: (BuildContext context) => AlertDialog(
      title: Text(
          '¿Estás seguro de que quieres borrar \" ${producto.info.nombre}\"?'),
      actions: <Widget>[
        TextButton(
            child: const Text('ACEPTAR'),
            onPressed: () {
              Navigator.pop(context, DeleteAction.agree);
            }),
        TextButton(
            child: const Text('CANCELAR'),
            onPressed: () {
              Navigator.pop(context, DeleteAction.cancel);
            })
      ],
    ),
  ).then((value) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        appBar: AppBar(
          ...
          actions: <Widget>[
            IconButton(
              ...
              color: Colors.white,
              onPressed: () {
                deleteProduct(widget.producto, context, widget.prefs,
                    widget.sumId, false);
              },
            )
          ],
        ...
        body: SingleChildScrollView(
          child: buildDetail(
              widget.producto, false, context, widget.prefs, widget.sumId),
        ));
  }
}
